...
if ( class_exists( 'CTA_Pro_Feature_Gate', false ) ) {
	return;
}

class CTA_Pro_Feature_Gate {
	 ...
	 *
	 * @return CTA_Pro_Feature_Gate
	 */
	public static function get_instance(): CTA_Pro_Feature_Gate {
	 ...
	 */
	private static function get_settings_repo(): CTA_Settings_Repository {
		return CTA_Settings_Repository::get_instance();
	}
	 ...
	 *
	 * Checks in order:
	 * 1. Plugin filter override (cta_manager_pro_enabled) - checked first for external control
	 * 2. Local license cache (24-hour TTL)
	 * 3. TopDevAmerica API validation if cache expired
	 * 4. Grace period (7 days) if API fails
	 *
	 * @return bool
	 */
	public static function is_pro_enabled(): bool {
		 ...
		 */
		$filter_result = apply_filters( 'cta_manager_pro_enabled', null );
		...
		if ( $api_result['grace_period_active'] ) {
			// Still within grace period (7 days)
			return true;
		}
	...
	private static function validate_license_via_api(): array {
		...
		$api_url = rest_url( 'cta-manager/v1/license/status' );
		$api_nonce = wp_create_nonce( 'wp_rest' );

		// Generate timestamp and signature headers for API request
		// Signature format: HMAC-SHA256(timestamp.body, shared_secret)
		// Reference: CTA_Manager_Portal::verify_api_signature()
		...
		$response = wp_remote_get(
			add_query_arg( 'license_key', urlencode( $license_key ), $api_url ),
			[
				'timeout'    => 5,
				'blocking'   => true,
				...
				'headers'    => [
					...
					'X-CTA-Timestamp'   => $timestamp,
					'X-CTA-Signature'   => $signature,
				],
			]
		);
		...
		if ( is_wp_error( $response ) ) {
			// API failed - check if we're still in grace period
		...
		if ( ! isset( $body['success'] ) || ! $body['success'] ) {
			// API returned error - check grace period
			$last_verified = $license_cache['last_verified'] ?? 0;
			$grace_period_end = $last_verified + ( 7 * DAY_IN_SECONDS );
	...
	private static function update_license_cache_data( array $license_data ): void {
		$cache = [
			...
			'expires_cache_at' => current_time( 'timestamp' ) + DAY_IN_SECONDS, // 24-hour cache TTL
		];

		// Use the new settings table method
	...
	public function is_feature_enabled( string $feature ): bool {
		// In free mode, all features are allowed when Pro is enabled.

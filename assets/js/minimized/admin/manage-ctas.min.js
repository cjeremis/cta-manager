...
/**
 * CTA Manager - CTA List Filters
 *
 * Handles filtering of CTA list items with URL parameter support
 * for shareable filtered URLs and state persistence on refresh.
 *
 * @package CTA_Manager
 * @since 1.0.0
 */
 ...
 *
 * Entry point for the Manage CTAs page (cta-manager-cta).
 * This page handles CTA creation, editing, filtering, and preview.
 *
 * When bundled with npm, this file will import:
 * - modals/format-modal   (CTCFormatModal)
 * - modals/preview-modal  (form preview functionality)
 * - forms/filters         (CTAFilters)
 *
 * Note: Edit functionality is now handled by global-modal.js which supports
 * both create and edit modes in a single modal.
 *
 * Dependencies (loaded via admin.js):
 * - CTCAdmin, ctaModalAPI, CTATabs
 *
...
(function ($) {
  ...
  /**
   * Manage CTAs Page Controller
   */
  window.CTAManageCTAsPage = window.CTAManageCTAsPage || {
    /**
    ...
    /**
     * Initialize the Manage CTAs page
     */
    init: function () {
    ...
    onAdminReady: function () {
      // Initialize format modal if available
      if (typeof window.CTCFormatModal !== 'undefined' && typeof window.CTCFormatModal.init === 'function') {
        // CTCFormatModal self-initializes
      }

      // Initialize filters if available
      if (typeof window.CTAFilters !== 'undefined' && typeof window.CTAFilters.init === 'function') {
        // CTAFilters self-initializes
      }
      ...
      $(document).trigger('ctaManageCTAsReady', [this]);
    },
    /**
    ...
    initCountdownTimers: function () {
      const self = this;
      const $countdowns = $('.cta-countdown-time[data-timestamp]');
      ...
      const updateCountdowns = function () {
        $countdowns.each(function () {
          ...
          const timestamp = parseInt($countdown.attr('data-timestamp'), 10);
          if (!timestamp) {
            return;
          }
          ...
          if (diff <= 0) {
            const expiredLabel = $countdown.attr('data-expired-label') || 'Now...';
